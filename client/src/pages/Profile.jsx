...
export default function Profile() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault(); //stops from refreshing the form even before updating anything
    try {
      ...
      const res = await fetch(`/api/user/update/${currentUser._id}`, {
        ...
        body: JSON.stringify({ ...formData, bio }), // Include bio in the request body
      });
  ...
  return (
    <div className='p-3 max-w-lg mx-auto' style={{ backgroundImage: 'linear-gradient(to bottom right, #3182CE, #8B5CF6)' }}>
    ...
    <form onSubmit={handleSubmit} className='flex flex-col gap-4'>
        ...
        <input
          ...
          onChange={handleChange}
        />
        <button className='bg-slate-700 text-white p-3 rounded-lg uppercase hover:opcaity-95 disabled:opacity-80'>
